...
function computeHitErrors(beatmap, replay, clockRate) {
    ...
    for (const f of replay.frames || []) {
        ...
        if (isPressed && !wasPressed) presses.push(f.startTime / clockRate);
        wasPressed = isPressed;
    }
    if (!presses.length) return null;
    if (circles.length * presses.length > MAX_MATCH_OPS) return null;
            ...
            if (osuRes.ok && osuText) {
                ...
                if (info.rulesetId === 0 && score.replay) {
                    try {
                        ...
                        result.hitErrors = computeHitErrors(parsedBeatmap, score.replay, clockRate);
                    } catch (err) {
