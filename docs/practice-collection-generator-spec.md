...
| 階段 | 內容 | 狀態 |
|---|---|---|
| **MVP** | 類型 1 突破分 + 類型 3 目標圖池 + 共用管線 | ✅ 上線（`e95d85e`, `e687769`） |
| v1.1 | 類型 2 低準度、類型 5 相似圖 | ✅ 上線（`0213436`, `fa3a483`） |
| v1.2 | 類型 4 弱項、回饋閉環 | ✅ 上線（`5179983`, `c1b4a61`） |
| Phase 2 | per-diff 分類成員、AR/CS/串圖弱項維度、`osu-pp` 精算重排序、弱項的長度上限（marathon 目前會混進來）、其餘 6 語系翻譯 | 待做 |
