...
const LOC = {
    ...
    cmd_practice: { en: 'Turn your top plays into a practice pool (.osdb, any mode)', 'zh-TW': '把你的 top play 變成一份練習圖池（.osdb，四模式）', 'zh-CN': '把你的 top play 变成一份练习图池（.osdb，四模式）', ja: '上位プレイから練習プールを作成（.osdb、全モード）', fr: 'Transformer tes meilleurs scores en pool d\'entraînement (.osdb, tous modes)', de: 'Aus deinen Top-Plays einen Übungspool machen (.osdb, alle Modi)', ru: 'Собрать тренировочный пул из лучших результатов (.osdb, любой режим)', 'es-ES': 'Convierte tus mejores scores en un pool de práctica (.osdb, cualquier modo)', ko: '상위 기록으로 연습 풀 만들기 (.osdb, 모든 모드)' },
