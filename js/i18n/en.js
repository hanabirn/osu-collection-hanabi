I18N.en = {
    ...
    nav_dan: 'Dan Courses',
    dan_page_title: '✦ Dan Courses ✦',
    dan_hint: "Skill-certification dan courses for each mode — community-made benchmark marathons, one map per dan. Led by each keymode's current project; other dan courses found by search sit under \"More\". Mania is 4K/7K only. Click a card to open it on osu!, + to add it to your collection.",
    dan_load_fail: 'Failed to load dan courses', dan_empty: 'No dan courses found for this mode', dan_load_more: 'Load more',
    dan_pinned: 'Curated',
    dan_add_group_btn: 'Add all of "{name}" to collection', dan_add_group_confirm: 'Create a collection category from the {n} sets in "{name}"?',
    dan_collection_name: 'Dan Courses - {mode}',
    dan_more_summary: 'More dan courses (from search, {n})', dan_more_note: "Other packs from osu!'s beatmap search whose title/artist look like a dan course - not hand-checked.",
    dan_standard_label: 'Passing standard', dan_resources_label: 'Resources',
    dan_std_mania: 'Clear the whole marathon in one sitting at 96%+ accuracy (95% on some intro dans); pausing fails the run, only Mirror/HD/FI/FL/HR/DT/NC allowed.',
    dan_std_osu: 'Clear the whole run in one sitting at 96%+ accuracy (Tapping Dan Course rule); pausing fails the run.',
    dan_std_taiko: "Judged by taiko's soul gauge - it must reach that dan's pass line by the end (varies per dan), roughly a high accuracy with no break.",
    dan_std_catch: 'Clear the whole marathon without draining out; some dans have an extra accuracy threshold - see the forum thread.',
