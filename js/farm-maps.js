...
// 3-column grid, 4 rows per page (see #farm-maps-list in css/osu.css). Sent
// to the server as `limit` (it already honours that override).
const FARM_PAGE_SIZE = 12;
...
function renderFarmMapsList() {
    ...
    listEl.innerHTML = farmItems.map(item => {
        ...
        <div class="osu-card" onclick="window.open('https://osu.ppy.sh/beatmapsets/${item.beatmapset_id}#${FARM_MODE_TO_API[farmMode]}/${item.beatmap_id}','_blank')">
            ...
            <button class="osu-play-btn" onclick="playOsuPreview(${item.beatmapset_id}, event)" title="${t('mappools_preview')}">${playBtnIcon()}</button>
